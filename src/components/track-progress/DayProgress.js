...
export const DayProgress = ({ day, rowIndex }) => {
  ...
  const { displayDuration, selectedGoal, dayProgress } =
    ...
    <>
      <div className={styles.progressDay}>
        <div>
          ...
          {selectedGoal.goal && (
            <>
              {dayjs(selectedGoal.createdAt).format("DD MM YYYY") ===
                currDay && (
                <DisplayStart
              ...
              {!dayProgress ? (
                ...
                <>
                  {dayProgress.map(
                    (day) =>
                      dayjs(day.createdAt).format("DD MM YYYY") === currDay && (
                        <DisplayProgress
                          key={day._id}
                          day={day}
                        ></DisplayProgress>
              ...
              {dayjs(displayDuration(selectedGoal._id)[0]).format(
                "DD MM YYYY"
              ) === currDay && (
                <DisplayEnd
                  key={selectedGoal._id}
                  goal={selectedGoal}
                  deadline={dayjs(displayDuration(selectedGoal._id)[0])}
                ></DisplayEnd>
              )}
            </>
          )}
