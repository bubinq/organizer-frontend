...
export const UpdateGoal = ({
  ...
  const invalidNotesHandler = (ev) => {
    ev.target.setCustomValidity(
      "List at least one goal-specific area to focus on"
    );
  };
    ...
    <Draggable handle="#handle">
      <div className={styles.eventModal}>
        <div className={styles.modalHeader}>
          <h3 className={styles.eventDay}>
            {dayjs(dayTarget).format("DD MMMM YYYY")}
          </h3>
        ...
        <form onSubmit={goalHandler} className={styles.eventForm}>
          <input
            ...
            defaultValue={dayInfo.goal}
            required
          ></input>

          <label htmlFor="notes" className={styles.labels}>
            Make a plan
          </label>
          <textarea
            name="notes"
            ...
            id="notes"
            maxLength={300}
            placeholder="List your notes each on new line"
            defaultValue={notes}
            onInvalid={invalidNotesHandler}
            onInput={validNotesHandler}
            required
          ></textarea>
          ...
          <div className={styles.labelWrapper} id="labels">
            {labelsArray.map((label, idx) => (
              <span
                ...
                onClick={() => selectLabelHandler(idx)}
              >
